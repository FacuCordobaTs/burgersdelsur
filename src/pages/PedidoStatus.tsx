...
/** Single-tenant Burgers del Sur (restaurante id en Piru). */
const TENANT_RESTAURANTE_ID = 13
 ...
 * Ruta: /pedido/:id
 * Single-tenant: siempre Burgers del Sur (username / id fijos).
 */
const PedidoStatus = () => {
    const { id } = useParams()
        ...
        const fetchOrderInfo = async () => {
            try {
                ...
                if (data.success && data.data) {
                    ...
                    if (restaurante?.id != null && restaurante.id !== TENANT_RESTAURANTE_ID) {
                        navigate('/')
                        return
                    }
    ...
    return (
        <div className="min-h-screen bg-background font-sans selection:bg-primary/20 pb-10 flex flex-col items-center">
            ...
            <MisPedidosDrawer
                ...
                restauranteId={restauranteData?.id ?? TENANT_RESTAURANTE_ID}
            />
        </div>
    )
}
